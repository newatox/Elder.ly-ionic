...
export class AuthProvider {
  ...
  private auth: User;
  private token: any;
  private reconnectionAlertVisible = false;
  ...
  */
  ...
  invalidToken() {
    if (this.network.isOnline()) {
      if (!this.reconnectionAlertVisible) {
        this.reconnectionAlertVisible = true;
        ...
        alert.onWillDismiss(() => {
          this.reconnectionAlertVisible = false;
        });
        alert.present().then();
      } else {
        console.log('Reconnection alert already visible');
      }
    } else {
